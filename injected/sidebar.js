...
// SIDEBAR
if (!localStorage.bancoBlackListed) localStorage.bancoBlackListed = [];
...
$(
  "<div id='TPLS'><div id='topicosPLS'></div><div id='menuTPLS'></div></div>"
).appendTo($("body"));
...
$(".post a").mouseover(function(e) {
  if (isURIImage(this.href) && displayPictures()) {
    if (isURIX(this.href, "webm")) {
      $(".followBox").html(
        '<video controls="" autoplay="" name="media"><source src="' +
          this.href +
          '" type="video/webm"></video>'
      );
    } else if (isURIX(this.href, "gifv")) {
      var tokens = /https?:\/\/i\.imgur\.com\/([a-zA-Z0-9]+).gifv/.exec(
        this.href
      );
      ...
      if (tokens.length === 2) {
        var gifvID = tokens[1];
        $(".followBox").html(
          '<video poster="https://i.imgur.com/' +
            gifvID +
            '.jpg" preload="auto" autoplay="autoplay" muted="muted" loop="loop" width="auto" height="auto"><source src="https://i.imgur.com/' +
            gifvID +
...
$(
  "<input id='filtro' type='search' placeholder='Tópicos Por Ler...'>"
).appendTo("#menuTPLS");
$(
  "<div id='actionLine'><div id='openSelected' class='botao'>Abrir Selecionados</div><div id='refreshTopics' class='botao'>↻</div></div>"
).appendTo("#menuTPLS");
...
$("#refreshTopics").on("click", function(e) {
  chrome.runtime.sendMessage({ "fetch-unread-topics": true }, function(
    response
  ) {
    if (response.success) {
      getBancoTopicosPorLer();
...
$("<ul id='resultadoPesquisa' style='display: block;'></ul>").appendTo(
  "#menuTPLS"
);
  ...
  chrome.runtime.sendMessage({ "get-unread-topics": true }, function(res) {
    ...
    for (var i = 0; i < topicos.length; i++) {
      var linha =
        topicos[i].name +
        " <span class='autor'>" +
        topicos[i].lastPoster +
        "</span>";
      $("ul#topicosPorLer").append(
        "<li data-topicLink='" +
          topicos[i].link +
          "'><div class='cont'>" +
          linha +
...
function displayEmentas() {
  getProximasEmentas(function(emnt) {
    ...
    if (emnt && emnt.length) {
      $("#ementa").html(
        "<div title='Almoço' id='almoco'></div><div title='Jantar' id='jantar'></div><div class='head'><div class='botao' id='ant'><</div><div id='dia'></div><div class='botao' id='seg'>></div></div>"
      );

      $("#ementa #ant").click(function() {
        if (indexEmentaActual > 0) indexEmentaActual--;
