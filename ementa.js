...
const diasSemana = ["Dom.", "Seg.", "Ter.", "Qua.", "Qui.", "Sex.", "Sab.", "Dom."];
...
function getDataProximaEmenta() {
    ...
    if (localStorage["calendarioEmenta"]) {
        ...
        for (i = 0; i < datas.length; i++) {
            var data = datas[i];
            if (data >= hoje) {
                res.push({ ementaA: calendario[data].almoco, ementaJ: calendario[data].jantar, data: data });
            }
        }
...
function getEmentas(onComplete) {
    ...
    var m = new Date(hoje);
    var hojeTXT = m.getFullYear() + "-" + ("0" + (m.getMonth() + 1)).slice(-2) + "-" + ("0" + m.getDate()).slice(-2); // YYYY-MM-DD
    var opts = "?singleEvents=true&timeMin=" + hojeTXT + "T00:00:00Z&orderBy=startTime&alwaysIncludeEmail=false&showDeleted=false&maxResults=250&key=" + APIKey;
    var pagAlmoco = "https://www.googleapis.com/calendar/v3/calendars/" + ementaAlmocoID + "@group.calendar.google.com/events" + opts;
    var pagJantar = "https://www.googleapis.com/calendar/v3/calendars/" + ementaJantarID + "@group.calendar.google.com/events" + opts;
...
function dataToRelativeString(data) {
    ...
    if (dif > 0 && dif < 7 * day) { // se for nos próximos 7 dias
