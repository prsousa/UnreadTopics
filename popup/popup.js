$(document).ready(function () {
    ...
    $("#filtro").on('search keyup', function (event) {
        ...
        $("#searchAnchor").attr("href", "http://lei-uminho.com/forum/?action=search2&search=" + searchedString);
    });
...
function htmlRowTopico(nomeTopico, idTopico) {
    return '<li class="buttonRow">\
                <a href="http://lei-uminho.com/forum/?topic=' + idTopico + '.msg9999999#new" target="_blank" class="popupButton hover" title="' + nomeTopico + '">\
    ...
    chrome.runtime.sendMessage({ "open-unread-topics": true }, function (response) {
        ...
        if (response.success) {
            ...
            if (response.unreadTopics.length === 0) {
                ...
                msg = response.unreadTopics.length + " Tópico" + plural + " Aberto" + plural;
            }
        } else {
        ...
        $("#report").show().html("<hr />").prepend(msg);
    });
}
...
function updatePostponedCountdown() {
    ...
    var text = (("0" + hours).slice(-2)) + ":" + (("0" + minutes).slice(-2)) + ":" + (("0" + seconds).slice(-2));
    $("#postponedCountdown").text(text);
}

function setIconNotifications() {
    $("#notificationsDisabledIcon, #postponedCountdown").hide();
    chrome.runtime.sendMessage({ "get-notifications-state": true }, function (notificationsState) {
...
function toggleNofiticationState() {
    if (!currentNotificationState.enabled || currentNotificationState.currentlyBlocked) {
        chrome.runtime.sendMessage({ "set-notifications-enabled": !currentNotificationState.enabled });
    } else {
...
function showEmenta() {
    ...
    toggleEverySeconds(3, { ementa: ementa.ementaA, momento: "almoço" }, { ementa: ementa.ementaJ, momento: "jantar" });
